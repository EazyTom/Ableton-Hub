...
class ViewManager(QObject):
    ...
    def __init__(self, content_stack: QStackedWidget, parent: Optional[QObject] = None):
    ...
    def switch_to_view(self, name: str) -> bool:
        ...
        if index >= self._stack.count():
            self.logger.warning(f"Index {index} out of bounds for view '{name}' (stack has {self._stack.count()} items)")
            return False
    ...
    def get_view_widget(self, name: str) -> Optional[QWidget]:
